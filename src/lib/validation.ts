...
export const profileSchema = z.object({
  ...
  public_email: z.union([z.literal(""), z.email().max(254)]), public_phone: z.string().trim().max(40), website: secureUrl,
...
export const importRowSchema = z.object({ email: z.email().max(254).transform((s) => s.trim().toLowerCase()), name: z.string().trim().min(1).max(120) }).strict();
export interface ImportRow { email: string; name: string }
...
export function parseAttendeeCsv(csv: string): { rows: ImportRow[]; errors: ImportIssue[] } {
  ...
  result.data.forEach((row, index) => {
    const parsed = importRowSchema.safeParse({ email: row.email?.trim().toLowerCase(), name: row.name });
